...
PG_FUNCTION_INFO_V1(update_formation);

bool SetFormationNumberSyncStanbys(const char *formationId, int numberSyncStanbys);
...
/*
 * update_formation update properties of a formation.
 */
Datum
update_formation(PG_FUNCTION_ARGS)
{
	text *formationIdText = PG_GETARG_TEXT_P(0);
	char *formationId = text_to_cstring(formationIdText);
	bool number_sync_stanbys_set = false;
	int number_sync_stanbys = -1;
	...
	if (!PG_ARGISNULL(1))
	{
		number_sync_stanbys_set = true;
		number_sync_stanbys = PG_GETARG_INT32(3);

		if (number_sync_stanbys < 0)
		{
			ereport(ERROR, (ERRCODE_INVALID_PARAMETER_VALUE,
							errmsg("invalid value for  number_sync_stanbys \"%d\""
								   "expected a non-negative integer", number_sync_stanbys)));
		}
	}

	if (number_sync_stanbys_set)
	{
		success = SetFormationNumberSyncStanbys(formationId, number_sync_stanbys);

		if (success)
		{
			formation->number_sync_stanbys = number_sync_stanbys;
		}
	}
...
/*
 * SetFormationNumberSyncStanbys sets numberSyncStandbys property
 * of a formation entry. Returns true if successfull.
 */
bool
SetFormationNumberSyncStanbys(const char *formationId, int numberSyncStanbys)
{
	Oid argTypes[] = {
	...
	Datum argValues[] = {
			Int32GetDatum(numberSyncStanbys), /* numberSyncStanbys */
			CStringGetTextDatum(formationId)  /* formationId */
	};
	const int argCount = sizeof(argValues) / sizeof(argValues[0]);
	int spiStatus = 0;
